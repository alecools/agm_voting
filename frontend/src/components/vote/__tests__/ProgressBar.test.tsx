...
describe("ProgressBar", () => {
  it("shows correct fraction", () => {
    ...
    expect(screen.getByLabelText("4 / 7 motions answered")).toBeInTheDocument();
  });

  it("shows 0/0 when total is zero", () => {
    render(<ProgressBar answered={0} total={0} />);
    expect(screen.getByLabelText("0 / 0 motions answered")).toBeInTheDocument();
  });

  it("shows all answered", () => {
    render(<ProgressBar answered={3} total={3} />);
    expect(screen.getByLabelText("3 / 3 motions answered")).toBeInTheDocument();
  });
