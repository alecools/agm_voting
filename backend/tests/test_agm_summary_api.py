...
from app.models import AGM, AGMStatus, Building, Motion
...
async def open_agm_with_motions(db_session: AsyncSession):
    """Building + open AGM with two motions (one has description, one does not)."""
    ...
    m1 = Motion(agm_id=agm.id, title="Motion One", order_index=1, description="Desc one")
    m2 = Motion(agm_id=agm.id, title="Motion Two", order_index=2, description=None)
...
async def closed_agm(db_session: AsyncSession):
    """Building + closed AGM with one motion."""
    ...
    m = Motion(agm_id=agm.id, title="Closed Motion", order_index=1, description="Details")
    db_session.add(m)
    await db_session.flush()
...
async def agm_no_motions(db_session: AsyncSession):
    """Building + open AGM with zero motions."""
...
async def agm_multiple_motions(db_session: AsyncSession):
    """Building + open AGM with motions inserted in reverse order to verify ordering."""
    ...
    m3 = Motion(agm_id=agm.id, title="Motion C", order_index=3)
...
class TestAGMSummary:
    ...
    async def test_open_agm_returns_correct_title(
        ...
        response = await client.get(f"/api/agm/{agm.id}/summary")
        data = response.json()
        assert data["title"] == "Open Summary AGM"
    ...
    async def test_nonexistent_agm_returns_404(self, client: AsyncClient):
        response = await client.get(f"/api/agm/{uuid.uuid4()}/summary")
        assert response.status_code == 404

    async def test_malformed_uuid_returns_422(self, client: AsyncClient):
        response = await client.get("/api/agm/not-a-uuid/summary")
